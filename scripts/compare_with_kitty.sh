...
OUT_FILE="$OUT_DIR/kitty_compare.txt"
mkdir -p "$OUT_DIR"
...
if [[ -z "$KITTY_BIN" ]]; then
  echo "kitty must be installed" >&2
  exit 2
fi
...
measure_window_cmd() {
  ...
  start_ns=$(date +%s%N)
  "$@" >/tmp/handterm-kitty-compare.log 2>&1 &
  launcher_pid=$!
  target_pid=""
...
measure_handterm_host() {
  ...
  local backend="$2"
  local tmpdir
  tmpdir=$(mktemp -d /tmp/handterm-kitty-compare.XXXXXX)
...
measure_kitty() {
  local app_id="kitty-compare"
  measure_window_cmd "kitty" "$app_id" "$KITTY_BIN" --class "$app_id"
}

: > "$OUT_FILE"
printf 'handterm vs kitty comparison (%s)\n\n' "$(date --iso-8601=seconds)" | tee -a "$OUT_FILE"
