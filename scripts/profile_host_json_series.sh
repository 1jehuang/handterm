...
mkdir -p "$OUT_DIR"
JSONL_FILE="$OUT_DIR/gpu_host_json_series_${STAMP}.jsonl"
SUMMARY_FILE="$OUT_DIR/gpu_host_json_series_${STAMP}.txt"
...
mkdir -p "$TMP_BASE"
TMPDIR="$(mktemp -d "$TMP_BASE/handterm-gpu-json-series.XXXXXX")"
...
wait_for_window_count() {
  ...
  for _ in $(seq 1 500); do
    if ! host_is_alive; then
      fail_with_host_log "gpu host exited before window count reached ${expected_count}"
    fi
    local count_now
...
wait_for_profile_event() {
  ...
  for _ in $(seq 1 500); do
    if ! host_is_alive; then
      fail_with_host_log "gpu host exited before ${event_name} for window ${window_id}"
    fi
...
jsonl_path.write_text("\n".join(json.dumps(obj, sort_keys=True) for obj in records) + ("\n" if records else ""))
...
def summarize(group_name, subset):
    ...
    for field in fields:
        ...
        if not values:
            continue
        lines.append(
            f'  {field}: min={min(values):.2f} median={statistics.median(values):.2f} max={max(values):.2f}'
        )
    lines.append('  ids=' + ', '.join(str(row['id']) for row in subset))
    return lines
