...
pub fn run(config: AppConfig, socket_path: PathBuf) -> Result<()> {
    ...
    let mut app = RemoteGpuApp::new(config, socket_path, proxy);
    event_loop
        .run_app(&mut app)
        .context("failed while running remote gpu app")
}

struct RemoteGpuApp {
    config: AppConfig,
    socket_path: PathBuf,
    state: Option<RemoteGpuState>,
...
impl ApplicationHandler<AppEvent> for RemoteGpuApp {
        ...
        match event {
            ...
            WindowEvent::Resized(size) => {
                if size.width > 0 && size.height > 0 {
                    let new_size = terminal_size_for_pixels(size.width, size.height, &state.atlas);
...
fn process_pending_io(state: &mut RemoteGpuState, event_loop: &ActiveEventLoop) -> RedrawWork {
...
fn apply_server_message(
    ...
    if let ServerMessage::AtlasUpdate { glyph } = message {
        ...
        state.renderer.last_presented_signature = None;
    }
...
fn send_paste(state: &mut RemoteGpuState, text: Vec<u8>) -> Result<()> {
    let Some(window_id) = state.window_id else {
        return Ok(());
    };
...
fn send_mouse_input(state: &mut RemoteGpuState, event: ProtocolMouseEvent) -> Result<()> {
    let Some(window_id) = state.window_id else {
        return Ok(());
    };
    state
